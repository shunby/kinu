...
use uefi::{boot::MemoryType, prelude::*, proto::media::file::{File, FileAttribute, FileInfo, FileMode}, CStr16};
...
fn load_kernel() -> uefi::Result<EntryPointFn>{
    ...
    let mut kernel_file = fs.open_volume()?.open(
        KERNEL_FILE_NAME,
        FileMode::Read,
        FileAttribute::empty())?.into_regular_file().expect("kernel.elf is not a regular file");
    
    // The size of FileInfo is 80 + filename
    let mut fileinfo_buf = [0u8; 128];
    let fileinfo: &FileInfo = kernel_file.get_info(&mut fileinfo_buf).expect("failed to get fileinfo of kernel.elf");
    ...
    let kernel_file_buf: &mut [u8] = 
        unsafe {
            ...
            let page_ptr = boot::allocate_pages(boot::AllocateType::AnyPages, MemoryType::LOADER_DATA, n_pages)?;
    ...
    for ph in kernel_elf.prog_headers.iter().filter(|h|h.p_type == elf::Elf64_PhdrType::PT_LOAD) {
        info!("[header] {:x} {:x} {:x} {:x} {:x}", ph.p_offset, ph.p_vaddr, ph.p_paddr, ph.p_filesz, ph.p_memsz);
        let (b, e) = ph.infile_range();
        let (b, e) = (b as usize, e as usize);
        let (b2, e2) = ph.inmem_range();
        let (b2, e2) = (b2 as usize - elf_range.0 as usize, e2 as usize - elf_range.0 as usize);
        let len = usize::min(e - b, e2 - b2);
        pages[b2..(b2+len)].copy_from_slice(&kernel_file_buf[b..b+len]);
        if e2 - b2 > e - b {
            pages[b2+len..e2].fill(0);
        }
        info!("mapping 0x{:x}:0x{:x} <- 0x{:x}:0x{:x}", b2 + elf_range.0 as usize, b2 + len + elf_range.0 as usize, b, e);
    }

    let entry_point = unsafe {
        core::mem::transmute(kernel_elf.elf_header.e_entry)
    };
